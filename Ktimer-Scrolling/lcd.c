...
// Existing LCD initialization and other code...

static struct timer_list lcd_scroll_timer;

void lcd_command(char command); 
static void lcd_scroll(struct timer_list *t); 
static void lcd_start_scroll(int direction);
static void lcd_stop_scroll(void);
static int scroll_direction = 1; // 1 for left, -1 for right
static int scroll_active = 0; // flag to indicate if scrolling is active
...
{
     del_timer_sync(&lcd_scroll_timer);  // Clean up the scroll timer
...
ssize_t lcd_write(struct file *pfile, const char __user *ubuf, size_t size, loff_t *poffset)
{
    ...
    lcd_clear_display(); // before sending data on to lcd clearing the exist data
...
static long lcd_ioctl(struct file *pfile, unsigned cmd, unsigned long param){
    ...
    case LCD_PRINT_ON_SECOND_LINE:
        ...
        break;
    case LCD_START_SCROLL_LEFT:
        lcd_start_scroll(1); // Start scrolling left
        printk(KERN_INFO "lcd_ioctl: Started scrolling left\n");
        break;
    case LCD_START_SCROLL_RIGHT:
        lcd_start_scroll(-1); // Start scrolling right
        printk(KERN_INFO "lcd_ioctl: Started scrolling right\n");
        break;
    case LCD_STOP_SCROLL:
        lcd_stop_scroll(); // Stop scrolling
        printk(KERN_INFO "lcd_ioctl: Stopped scrolling\n");
        break;
    default:
...
static void lcd_print(char *msg, unsigned int lineNumber)
{
	...
	if( lineNum == 1 )
	{
		lcd_set_line_position( LCD_LINE_NUM_ONE );
		while( *(msg) != '\0' )
		{
	...
	if( lineNum == 2)
	{
		lcd_set_line_position( LCD_LINE_NUM_TWO);
		while( *(msg) != '\0' )
		{
			if(counter >=  NUM_CHARS_PER_LINE )
			{
...
// Function to scroll LCD
static void lcd_scroll(struct timer_list *t) {
    if (scroll_active) {
        // Shift display left or right
        if (scroll_direction == 1) {
            lcd_command(LCD_CMD_SHIFT_DISPLAY_LEFT);
        } else {
            lcd_command(LCD_CMD_SHIFT_DISPLAY_RIGHT);
        }
        mod_timer(&lcd_scroll_timer, jiffies + 1 * HZ); // Schedule next scroll in 1 second
    }
}
...
MODULE_DESCRIPTION("This kernel module of BBB for LCD");
